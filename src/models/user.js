...
userModel.goLive = (params,callback)=>{ 
        ...
        var chat;
        if(params.channel == 'Conectate'){
            chat = true;
        }else{
            chat = false;
        }
        var _dataOn = {Idvivo:params.ID_video,Vivo:chat,chatEvent:params.topic}
...
userModel.payu = (params,callback)=>{ 
    console.log(params)
    cvivoMain.firestore().collection("payu").add({param:params.test}).then(()=> {callback(null,'En vivo')})
}
